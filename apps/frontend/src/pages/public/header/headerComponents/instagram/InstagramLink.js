...
import '@/pages/public/header/headerComponents/instagram/InstagramButton.css';
import { InstagramIcon } from '@/pages/public/header/headerComponents/instagram/InstagramIcon';
export function InstagramLink({ username = 'a.sasha.art' }) {
    const openInstagram = () => {
        const webUrl = `https://www.instagram.com/${username}/`;
        const iOSDeeepLink = `insagram://user?username=${username}`;
        const androidIntent = `intent://instagram.com/_u/${username}` +
    ...
    return (_jsx("button", { type: "button", onClick: openInstagram, className: "btn instagramButton", "aria-label": "Instagram", children: _jsx(InstagramIcon, { size: 24 }) }));
}
