...
function pickThumbUrl(preview) {
    if (!preview)
        return '';
    return preview.jpeg ?? preview.webp ?? preview.avif ?? '';
}
function toGridSources(preview) {
    if (!preview)
        return undefined;
    const s = {};
    if (preview.avif)
...
export function applyArtCatalogFilter(itemsById, filter) {
    const q = norm(filter.query ?? '');
    const items = Object.values(itemsById);
    const filtered = items.filter((it) => {
        // query match: id + localized title + notes
        if (q) {
            const title = localizedToText(it.title);
        ...
        // technique
        if (filter.technique) {
            if (!(it.techniques ?? []).includes(filter.technique))
                return false;
        }
        // availability
        if (filter.availability) {
            if (String(it.availability) !== String(filter.availability))
                return false;
        }
        // series
        if (filter.series) {
            if ((it.series ?? '') !== filter.series)
                return false;
        }
        // has price
        if (filter.hasPrice) {
