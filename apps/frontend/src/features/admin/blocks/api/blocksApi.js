...
export async function createCollection(name) {
    ...
    if (!resp.ok)
        throw new Error(`Failed to create new collection ${name}`);
    return await resp.json();
}
...
export async function addNewBlock(block) {
    ...
    if (!res.ok)
        throw new Error(`Add new block error: ${res.status}`);
...
export async function updateBlock(block) {
    ...
    if (!res.ok)
        throw new Error(`Update block error: ${res.status}`);
    return await res.json();
}
export async function deleteBlock(id) {
    ...
    if (!res.ok)
        throw new Error(`Delete block error: ${res.status}`);
    return res.status;
}
