...
import { addNewBlock, deleteBlock, getCollection, updateBlock, } from '@/features/admin/blocks/api/blocksApi';
...
import { useArrival, useDispatch, usePeekTicket, useReturnHome, } from '@/features/admin/shared/transporter/transporter';
import { deepEqual } from '@/shared/lib/checkers/checkers';
import { editSessionsDataStore, unsavedChangesStore, useSessionDataStore, useUnsavedChanges, } from '@/shared/state';
...
import { isBlockReturnCommand, validateBlockReturnBootstrapInsertArt, } from './bootstrap/blockEditorSession.bootstrap.validate';
export function BlockEditorSessionProvider({ children }) {
    // Global context
    const gCtxt = useEditorWorkspace();
    //*******************************************************/
    // Core state:
    ...
    const [modeStack, setModeStack] = useState(['select']);
    ...
    const isSelected = useMemo(() => modeStack[modeStack.length - 1] !== 'select' && !!draft?.id, [modeStack, draft]);
    const scope = useMemo(() => {
        if (!isSelected || !draft)
            return null;
    ...
    const isEditingTarget = useCallback((t) => {
        if (!currentTarget || !t)
            return false;
        if (currentTarget.blockKind !== t.blockKind)
            return false;
        if (currentTarget.kind !== t.kind)
            return false;
            ...
            switch (ticket.phase) {
                ...
                case 'return': {
                    ...
                    if (!isBlockReturnCommand(effect)) {
                        throw new Error(`[Bootstrap]: Unexpected returnEffect for BlockEditor: ${effect ? effect.kind : 'undefined'}`);
                    }
                    ...
                    if (!blockId)
                        throw new Error('[Bootstrap]: returnEffect missing blockId');
                    setSelectedBlockId(blockId);
                    // ❗️ New approach: just created block hasn't get into collection yet!
                    // The true place to get the block - external store!!!
                    const key = { kind: 'block', id: blockId };
                    const block = editSessionsDataStore.get(key);
                    if (!block)
                        throw new Error(`[Bootstrap]: Journey ticket block doesn't stored in SessionsDataStore`);
...
*/
    ...
    const save = useCallback(async () => {
        ...
        try {
            ...
            switch (draft?.lifecycle) {
                ...
                default:
                    console.error(`Unexpected draft.lifecycle or undefined draft while saving ${draft.lifecycle}`);
            }
            commit();
        }
        catch (err) {
        ...
        if (saved)
            console.log(`[save]: block saved and new id received: ${saved.id}`);
        if (!saved)
            console.log(`[save]: block saved and id stay: ${draft.id}`);
        return { ok: true, id: saved ? saved.id : draft.id };
    }, [draft, canSave, refreshCollection, commit]);
    // ******** After save processing ********
    const finalizeAfterSave = useCallback((savedId) => {
        ...
        if (ticket?.phase === 'return' && ticket.destination.editor === 'block') {
            console.log(`[finalizeAfterSave]: taken decision to return home with blockId: ${savedId}`);
    ...
    const exit = useCallback(() => {
        if (saving)
            return;
        if (isDirty && !confirm('Discard unsaved block changes?'))
            return;
        resetSession();
    ...
    const onDelete = useCallback(() => {
        void (async () => {
            ...
            if (!draft || !draft.id || draft.lifecycle !== 'saved')
                return;
    ...
    const handleEditHit = useCallback((hit) => {
        // 🚧 Construction Yard:
        ...
        */
        ...
        console.dir(hit);
        if (currentStack.screenMode !== 'edit')
            return;
    ...
    const stackNewDraft = (b) => {
        const key = { kind: 'block', id: b.id };
        editSessionsDataStore.saveDraft(key, b);
        editSessionsDataStore.commit(key);
    };
    // select mode onBlock click handler
    const handleSelectHit = useCallback((hit) => {
        console.log(`[BlockEditorSessionProvider][handleSelectHit]: Called`);
        switch (hit.block.lifecycle) {
            case 'draft': {
                console.log(`[BlockEditorSessionProvider][handleSelectHit]: Called`);
                console.warn(`[handleSelectHit]: Block ${hit.block.id} has inappropriate lifecycle type 'draft'!`);
                break;
            }
            case 'saved': {
                console.log(`[BlockEditorSessionProvider][handleSelectHit]: Saved lifecycle detected`);
            ...
            case 'template': {
                console.log(`[BlockEditorSessionProvider][handleSelectHit]: template branch selected`);
    ...
    const onHit = useCallback((hit) => {
        if (currentStack.screenMode === 'select')
            handleSelectHit(hit);
        else
            handleEditHit(hit);
    ...
    return _jsx(BlockEditorCtx.Provider, { value: value, children: children });
}
