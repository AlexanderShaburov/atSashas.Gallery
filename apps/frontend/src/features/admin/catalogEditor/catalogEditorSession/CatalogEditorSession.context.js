...
import { getCatalog, getDependents, getSeriesOptionsCI, getTechniques, updateCatalog, } from '../api';
const Ctx = createContext(undefined);
// eslint-disable-next-line react-refresh/only-export-components
export const useCatalogEditorSession = () => {
    const v = useContext(Ctx);
    if (!v)
        throw new Error('useEditorSession must be used within CatalogEditorSessionProvider');
    return v;
};
export function CatalogEditorSessionProvider({ children }) {
    ...
    const sessionData = useSessionDataStore(scope);
    ...
    const isValid = useMemo(() => {
        if (!draft || !selectedItemId || !isSelected)
            return false;
    ...
    const resetSession = useCallback(() => void (async () => {
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                console.error('[CatalogEditorSessionProvider]: Failed to load techniques/options:', err);
            }
        })();
        return () => {
            alive = false;
    ...
    */
                ...
                case 'outbound': {
                    ...
                    switch (ticket.destination.mode) {
                        ...
                        case 'edit': {
                            ...
                            if (!item)
                                throw new Error(`[Catalog BOOTSTRAP]: ArtItem not found in catalog: ${id}`);
                            setSelectedItemId(id);
                            setScreenMode('edit');
                            return;
                        }
                        default:
                            throw new Error('[Catalog BOOTSTRAP]: Unsupported outbound destination.mode');
                    }
                }
                case 'return': {
                    /*❗️
    ...
    const editById = useCallback((id) => {
        ...
        const key = { kind: 'catalog', id };
        editSessionsDataStore.saveDraft(key, item);
    ...
    const deleteById = useCallback((_id) => {
    ...
    const save = useCallback(async () => {
        ...
        try {
            ...
            if (code !== 200)
                throw new Error(`Catalog update unsuccessful! Code: ${code}`);
    ...
    const onDraftChange = useCallback((next) => setDraft(next), [setDraft]);
